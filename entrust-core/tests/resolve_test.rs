...
fn test_resolve_existing() -> anyhow::Result<()> {
    let test_store = setup_test_store()?;
    let file1 = entrust_core::resolve_existing_location(test_store.path(), "file1", false)?;
    assert_eq!(test_store.path().join("file1"), file1);

    let dir1 = entrust_core::resolve_existing_location(test_store.path(), "dir1", true)?;
    assert_eq!(test_store.path().join("dir1"), dir1);
    assert!(entrust_core::resolve_existing_location(test_store.path(), "dir1", false).is_err());

    let dir1_file1 =
        entrust_core::resolve_existing_location(test_store.path(), "dir1/file1", false)?;
    assert_eq!(test_store.path().join("dir1/file1"), dir1_file1);

    let dir2 = entrust_core::resolve_existing_location(test_store.path(), "dir2", true)?;
    assert_eq!(test_store.path().join("dir2"), dir2);
    let dir2_pass = entrust_core::resolve_existing_location(test_store.path(), "dir2", false)?;
    assert_eq!(test_store.path().join("dir2/pass"), dir2_pass);

    assert!(
        entrust_core::resolve_existing_location(test_store.path(), "no such file", true).is_err()
    );
...
fn test_resolve_new() -> anyhow::Result<()> {
    ...
    let new = entrust_core::resolve_new_location(test_store.path(), "new")?;
    assert_eq!(test_store.path().join("new"), new);

    let new_in_new_dir = entrust_core::resolve_new_location(test_store.path(), "new_dir/new")?;
    assert_eq!(test_store.path().join("new_dir/new"), new_in_new_dir);

    assert!(entrust_core::resolve_new_location(test_store.path(), "file1").is_err());
